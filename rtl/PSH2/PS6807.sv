...
`endif
	...
	//Palette
	wire [10: 0] PALR_RA = PALETTE_SEL && !RD_N ? A[12:2] : BG_COLOR;
	wire [10: 0] PALG_RA = PALETTE_SEL && !RD_N ? A[12:2] : BG_COLOR;
	wire [10: 0] PALB_RA = PALETTE_SEL && !RD_N ? A[12:2] : BG_COLOR;
	wire         PAL_WE = PALETTE_SEL & ~(&WE_N) & WE_N_OLD;
	bit  [23: 0] PAL_Q;
	PSH2_PAL_RAM PALR(CLK, {1'b0,A[12:2]}, DI[15: 8], PAL_WE & ~A[1] & ~WE_N[1] & CE, {1'b0,PALR_RA}, PAL_Q[23:16]);
	PSH2_PAL_RAM PALG(CLK, {1'b0,A[12:2]}, DI[ 7: 0], PAL_WE & ~A[1] & ~WE_N[0] & CE, {1'b0,PALG_RA}, PAL_Q[15: 8]);
	PSH2_PAL_RAM PALB(CLK, {1'b0,A[12:2]}, DI[15: 8], PAL_WE &  A[1] & ~WE_N[1] & CE, {1'b0,PALB_RA}, PAL_Q[ 7: 0]);
				...
				if (SPR_LOAD_RUN) begin
					...
					if (SPR_LOAD_STATE == 3'h5) begin
						...
						if (SPR_LIST_NUM == 11'h5FF || SPR_NUM[14]) begin
							...
							if (!SPR_NUM[15] && SPR_NUM[13] == 1'b0) SPR_LOAD_NUM <= SPR_LOAD_NUM + 10'd1;
						end
					end
	...
	//Sprite fetch
	//Cycle0
	//H(4), Y(8), W(4), X(10), FLIPY(1), FLIPX(1), PAL(6), TN(19)
	wire [76: 0] SPRITE_PARAM_DATA = {23'b00000000000000000000000,SPR_EVAL_LIST.H,SPR_EVAL_LIST_Y_Q[7:0],SPR_EVAL_LIST.W,SPR_EVAL_LIST.X,SPR_EVAL_LIST.FLIPY,SPR_EVAL_LIST.FLIPX,SPR_EVAL_LIST.PAL,SPR_EVAL_LIST.TN};
	...
	wire [ 8: 0] SPR_LINE0_WA   = !SPR_DRAW_LINE2 ? SPR_LINE_X2[8:0] : DISP_X;
	wire [20: 0] SPR_LINE0_DATA = !SPR_DRAW_LINE2 ? SPR_LINE_DIN : 21'h0;
	wire         SPR_LINE0_WE   = !SPR_DRAW_LINE2 ? SPR_LINE_WREN : DOT_CE_R;
	PSH2_SPR_LINE SPR_LINE0(CLK, SPR_LINE0_WA, SPR_LINE0_DATA, SPR_LINE0_WE, DISP_X, SPR_LINE_Q[0]);
	
	wire [ 8: 0] SPR_LINE1_WA   =  SPR_DRAW_LINE2 ? SPR_LINE_X2[8:0] : DISP_X;
	wire [20: 0] SPR_LINE1_DATA =  SPR_DRAW_LINE2 ? SPR_LINE_DIN : 21'h0;
	wire         SPR_LINE1_WE   =  SPR_DRAW_LINE2 ? SPR_LINE_WREN : DOT_CE_R;
	PSH2_SPR_LINE SPR_LINE1(CLK, SPR_LINE1_WA, SPR_LINE1_DATA, SPR_LINE1_WE, DISP_X, SPR_LINE_Q[1]);
	...
	always @(posedge CLK or negedge RST_N) begin	
		if (!RST_N) begin
			...
			if (DOT_CE_R) begin
				...
				SPR_OUT_PAL <= SPR_LINE_DOUT[13:8];
			end
		end
	end
	...
	bit  [10: 0] BG_COLOR;
	always_comb begin
		BG_COLOR <= {SPR_OUT_PAL,5'b00000} + {3'b000,SPR_OUT_PIX};
	end
		...
		if (!RST_N) begin
			...
			if (DOT_CE_R) begin
				if (SPR_OUT_PIX)
					...
					OUT_RGB <= SCREEN_BG_COL[0];
			end
		end
	end
	assign {R,G,B} = RGBBright(OUT_RGB, 8'h80 - (SCREEN_BRIGHT[0] & 8'h7F));
